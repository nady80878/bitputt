...
class Validation{
	...
	public function check($source , $items = []){
		if(count($items)){
			...
			foreach ($items as $item => $rules) {
					$item = Func::escape(trim($item));
					$item_value = Func::escape(trim($source[$item]));
					...
					switch ($rule) {
						...
						case 'min':
							if($rule_value && strlen($item_value) < $rule_value){
								$this->addError($item,"{$item} must be at least {$rule_value} chars!");
							}
							break;
						case 'max':
							if($rule_value && strlen($item_value) > $rule_value){
								$this->addError($item,"{$item} must be maximum {$rule_value} chars!");
							}
							break;
						case 'matches':
							if($rule_value&& strcmp($item_value , $source[$rule_value]) != 0){
								$this->addError($item,"{$item} & {$rule_value} don't match!");
							}
							break;
						case 'equals':
							if(count($rule_value)&& !in_array($item_value , $rule_value)){
								$this->addError($item,"{$item} must be one of  [ ".implode(', ',$rule_value)." ]!");
							}
							break;
						case 'alpha':
							if($rule_value === true&& !ctype_alpha($item_value)){
								$this->addError($item,"{$item} must be alphabetic chars!");
							}
							break;
						case 'alpha_space':
							if($rule_value === true&& !preg_match('/^[ a-zA-Z]+$/' ,$item_value,$matches)){
								$this->addError($item,"{$item} must be alphabetic chars and spaces! ");
							}
							break;
						case 'num':
							if($rule_value === true&& !ctype_digit($item_value)){
								$this->addError($item,"{$item} must be numeric chars!");
							}
							break;
						case 'alpha_num':
							if($rule_value === true&& !preg_match('/(?:[a-zA-Z]+[0-9 ]+)|(?:[0-9 ]+[a-zA-Z]+)/',$item_value)){
								$this->addError($item,"{$item} must contain alphabetic and numeric chars!");
							}
							break;
						case 'regexp':
							if($rule_value&& !preg_match($rule_value,$item_value)){
								$this->addError($item,"{$item} must be matches this pattern {$rule_value} !");
							}
							break;
						case 'unique':
							if($rule_value){
							...
							switch ($rule_value) {
								case 'password':
									if(!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*(_|[^\w])).+$/', $item_value)){
										$this->addError($item,"The password must contains at least one capital letter, one small letter, one digit, and one special character!");
									}
									break;
								case 'username':
									if(!preg_match('/^[a-zA-Z0-9_-]{3,20}$/', $item_value)){
										$this->addError($item,"The username may contains alphanumeric, dashes and underscores only , min = 3 and max = 20!");
									}
									break;
								case 'url':
									if(!preg_match('/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/', $item_value)){
										$this->addError($item,"The url is invalid!");
									}
									break;
								case 'color':
									if(!preg_match('/^#?([a-f0-9]{6}|[a-f0-9]{3})$/', $item_value)){
										$this->addError($item,"The color is invalid!");
									}
									break;
								case 'ip':
									if(!preg_match('/^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/', $item_value)){
										$this->addError($item,"The ip is invalid!");
									}
									break;
								case 'tag':
									if(!preg_match('/^<([a-z]+)([^<]+)*(?:>(.*)<\/\1>|\s+\/>)$/', $item_value)){
										$this->addError($item,"The tag is invalid!");
									}
									break;
								case 'email':
									if(!preg_match('/([\w-\.]+)@((?:[\w]+\.)+)([a-zA-Z]{2,4})/', $item_value)){
										$this->addError($item,"The email must be a valid one!");
									}
									break;
							}
							break;
	...
	public function getErrors($item = ''){
		if(!empty($item) && key_exists($item, $this->_errors)){
